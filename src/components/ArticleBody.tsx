      ...
      {blocks.map((block, index) => {
        ...
        if (block.type === "quote") {
          return (
            <blockquote
              key={index}
              className="my-6 border-l-4 border-gold bg-paper-2 px-5 py-4 font-serif text-lg leading-relaxed"
            >
              <RichText text={block.text} />
              {block.cite ? (
                <cite className="mt-2 block font-sans text-sm not-italic text-ink-soft">{block.cite}</cite>
              ) : null}
            </blockquote>
          );
        }
