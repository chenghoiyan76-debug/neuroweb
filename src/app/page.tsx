    ...
    <div>
      <section className="mesh-hero text-paper-2">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 md:grid-cols-[1.15fr_0.85fr] md:py-24">
          <div>
            <p className="text-sm tracking-[0.18em] text-paper-2/70">{t.heroLead}</p>
            <h1 className="mt-4 font-serif text-4xl leading-tight md:text-5xl">
              {site.name}
              <span className="mt-3 block text-2xl font-normal md:text-3xl">{site.nameZh}</span>
            </h1>
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-paper-2/85">
              {site.tagline[locale]}
            </p>
            <p className="mt-4 max-w-xl text-sm leading-relaxed text-paper-2/70">{t.heroBody}</p>
          ...
          <aside className="rounded-3xl border border-white/15 bg-white/8 p-6 backdrop-blur-sm">
            <p className="text-sm text-paper-2/70">{t.fiveAxes}</p>
            <ol className="mt-4 grid gap-3 text-sm leading-relaxed">
              {axes.map((axis) => (
                <li key={axis.slug}>
                  <Link href={axis.href} className="hover:underline">
                    <span className="font-medium">{bilingualTitle(axis.zh, axis.en, locale)}</span>
                  </Link>
                </li>
              ))}
            </ol>
      ...
      <section className="paper-grid border-b border-rule">
        <div className="mx-auto max-w-6xl px-4 py-14">
          <h2 className="font-serif text-3xl">{t.coreNav}</h2>
